...
# Build
go build -o document-service ./cmd/server

# Run (uses all CPUs for parallel OCR by default)
./document-service

# Run with limited parallelism
GOMAXPROCS=2 OMP_NUM_THREADS=1 ./document-service
...
| `EPUB_RETENTION_HOURS` | `24` | Hours before generated EPUBs are deleted by the cleanup loop (requires `OUTPUT_DIR`). |
