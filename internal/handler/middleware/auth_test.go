...
func TestBasicAuthEmptyPassword(t *testing.T) {
	cfg := &config.SecurityConfig{
		BasicAuth: "admin:", // Invalid config - should pass through
	}
	...
	// Empty password in config is invalid, should pass through
	if !nextCalled {
		t.Error("expected next handler to be called with invalid config")
	}
}

func TestBasicAuthEmptyUsername(t *testing.T) {
	cfg := &config.SecurityConfig{
		BasicAuth: ":password", // Invalid config - should pass through
	}
	...
	// Empty username in config is invalid, should pass through
	if !nextCalled {
		t.Error("expected next handler to be called with invalid config")
	}
}

func TestBasicAuthNoColon(t *testing.T) {
	cfg := &config.SecurityConfig{
		BasicAuth: "invalid_no_colon", // Invalid config - should pass through
	}
	...
	// No colon in config is invalid, should pass through
	if !nextCalled {
		t.Error("expected next handler to be called with invalid config")
	}
}
